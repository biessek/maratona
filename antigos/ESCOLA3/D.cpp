...
bool compp(const point &a, const point &b) {
...
int main() {
    int N, i, j, ip = -1;   
    point po, ps, a, b; 
    double xi, X, m=-1., mx=-1, lx = 1000000;
    scanf("%d %lf", &N, &X);
        a.y = 0; b.y = 0; a.x = 0; b.x = double(X);
    for(i=0;i<N;i++) {
        scanf("%lf %lf", &pt[i].x, &pt[i].y);
        double d = distToLineSegment(pt[i],a, b, po);
        if(cmp(pt[i].x,mx) > 0) mx=pt[i].x;
        if(cmp(pt[i].x,lx) < 0) lx=pt[i].x;
    ...
    for(i=0;i<100;i++) {
        ...
        double d = hypot(pt[ip].x-xi,pt[ip].x-0);
    printf("%.2lf %.2lf\n", xi, d);  
}

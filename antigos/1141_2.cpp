...
const int MAXS = 1000010; // Max number of states in the matching machine.
...
int count[MAXS]; // Goto function, or -1 if fail.
...
int buildMatchingMachine(const vector<string> &words, char lowestChar = 'a', char highestChar = 'z') {
    ...
    for (int i = 0; i < words.size(); ++i) {
        ...
        count[currentState]++; // There's a match of keywords[i] at node currentState.
    }
    ...
    while (q.size()) {
        ...
        for (int c = 0; c <= highestChar - lowestChar; ++c) {
            if (g[state][c] != -1) {
                ...
                failure = g[failure][c];
                f[g[state][c]] = failure;
                count[g[state][c]] = max(count[failure],count[g[state][c]]+1);
                q.push(g[state][c]);
            }
        }
    }
...
int main(){
    ...
    while(scanf("%d", &N) && N) {
        ...
        for(int i =0;i<N;i++) {
            scanf(" %s", l);
            keywords.push_back(string(l));
        }
        int s = buildMatchingMachine(keywords, 'a', 'z');
        for(int i=0;i<s;i++) {
            printf("%d , %d\n",i,count[i]);
        }
    }
